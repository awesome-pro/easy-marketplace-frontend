...
// Extend Window interface to include resetFileUpload
declare global {
  interface Window {
...
export function FileUpload({
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    const validFiles = acceptedFiles.filter(file => {
      if (file.size > maxSize) {
        toast.info(`${file.name} exceeds ${formatFileSize(maxSize)} limit`);
        return false;
      }
      return true;
    });
  ...
  }, [maxSize, autoUpload, fileUploadContext]);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (failedCount > 0) {
        toast.info(`${failedCount} file(s) failed to upload. Please try again.`);
